...
void dump_to_file(const char *file){
    ...
    MPI_File_set_view(fout, offset*sizeof(int), MPI_INT, MPI_INT, "native", MPI_INFO_NULL);
    MPI_File_write_all(fout, data + offset, len, MPI_INT, MPI_STATUS_IGNORE);
...
void block_FW(){
...

    {
        ...
        if(count == 2)
            ...
            cudaSetDevice(0);
        cudaMallocPitch(&device_ptr, &pitch_bytes, vert_bytes, vert);
        cudaMemcpy2DAsync(device_ptr, pitch_bytes, data, vert_bytes, vert_bytes, vert, cudaMemcpyHostToDevice);
        ...
        if(num==0){  //G11 update
            ...
            for(int r=0;r<Round_0;++r){
                phase_one<BLOCK_SIZE><<< 1 , dimt >>>(device_ptr, pitch, pivot, sp[0]);
                phase_two<BLOCK_SIZE><<< p2b , dimt >>>(device_ptr, pitch, pivot, 0, sp[0], r);   //0~sp[0]
                phase_three<BLOCK_SIZE><<< p3b , dimt >>>(device_ptr, pitch, pivot, 0, sp[0], sp[0], r); //0~sp[0]
                pivot += BLOCK_SIZE;
            }
        }else{  //G11 update
            int pivot = 0;
        ...
        if(num == 0){ //G11 -> G12
            int pivot = 0;
            for(int r=0;r<Round_0;++r){
                phase_two_h<BLOCK_SIZE><<< p2b_1 , dimt >>>(device_ptr, pitch, pivot, sp[0], sp[0], vert, sp[0]);
                phase_three_h<BLOCK_SIZE><<< p3b_g12, dimt >>>(device_ptr, pitch, pivot, 0, sp[0], sp[0], vert, sp[0], r);
                pivot += BLOCK_SIZE;
            }

            cudaMemcpy2D(data, vert_bytes, 
                        device_ptr, pitch_bytes,
                        vert_bytes, sp[0], cudaMemcpyDeviceToHost);

            LOG("[mpi %d] send to %d", 0, 1);
            MPI_Sendrecv(data, vert*sp[0], MPI_INT, 1, 1, 
                        data+sp[0]*vert, vert*sp[1], MPI_INT, 1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

            LOG("[mpi %d] recv from %d", 0, 1);
            cudaMemcpy2DAsync(device_ptr+sp[0]*pitch, pitch_bytes,
                        data+sp[0]*vert, vert_bytes,
                        vert_bytes, sp[1], cudaMemcpyHostToDevice);

            //cudaMemcpy2D(device_ptr[1], pitch_bytes[1],
            ...
            int pivot = 0;
            for(int r=0;r<Round_0;++r){
                phase_two_v<BLOCK_SIZE><<< p2b_1 , dimt >>>(device_ptr, pitch, pivot, sp[0], vert, sp[0], sp[0]);
                phase_three_v<BLOCK_SIZE><<< p3b_g21 , dimt >>>(device_ptr, pitch, pivot, sp[0], 0, vert, sp[0], sp[0], r);
                pivot += BLOCK_SIZE;
            }

            cudaMemcpy2D(data + sp[0]*vert, vert_bytes, 
                        device_ptr +sp[0]*pitch, pitch_bytes,
                        vert_bytes, sp[1], cudaMemcpyDeviceToHost);

            LOG("[mpi %d] send to %d", 1, 0);
            MPI_Sendrecv(data + sp[0]*vert, vert*sp[1], MPI_INT, 0, 1,
                        data, vert*sp[0], MPI_INT, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            LOG("[mpi %d] recv from %d", 1, 0);
            cudaMemcpy2DAsync(device_ptr, pitch_bytes,
                        data, vert_bytes,
                        vert_bytes, sp[0], cudaMemcpyHostToDevice);
        ...
        if(num == 0){  //G22 relax
            ...
            int pivot = 0;
            for(int r=0;r<Round_0;++r){
                phase_three<BLOCK_SIZE><<< p3b_g22 , dimt >>>(device_ptr, pitch, pivot, sp[0], vert, sp[0], vert);
                pivot += BLOCK_SIZE;
            }  
        }else{ //G22 relax
        ...
        if(num == 0){  //G22 update
            ...
            for(int r=0;r<Round_1;++r){
                phase_one<BLOCK_SIZE><<< 1 , dimt >>>(device_ptr, pitch, pivot, vert);
                phase_two<BLOCK_SIZE><<< p2b , dimt >>>(device_ptr, pitch, pivot, sp[0], vert, r);  //sp[0]~vert
                phase_three<BLOCK_SIZE><<< p3b , dimt >>>(device_ptr, pitch, pivot, sp[0], vert, vert, r);//sp[0]~vert
                pivot += BLOCK_SIZE;
            }
        }else{  //G22 update
            int pivot = sp[0];
        ...
        if(num == 0){  //G22 -> G12
            ...
            int pivot = sp[0];
            for(int r=0;r<Round_1;++r){
                phase_two_v<BLOCK_SIZE><<< p2b_0 , dimt >>>(device_ptr, pitch, pivot, 0, sp[0], vert, vert);
                phase_three_v<BLOCK_SIZE><<< p3b_g12 , dimt >>>(device_ptr, pitch, pivot, 0, sp[0], sp[0], vert, vert, r);
                pivot += BLOCK_SIZE;
            }

            LOG("[mpi %d] wait receive from %d", 0, 1);
            MPI_Recv(data+sp[0]*vert, vert*sp[1], MPI_INT, 1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            LOG("[mpi %d] recv from %d", 0, 1);
            ...
            int pivot = sp[0];
            for(int r=0;r<Round_1;++r){
                phase_two_h<BLOCK_SIZE><<< p2b_0 , dimt >>>(device_ptr, pitch, pivot, 0, vert, sp[0], vert);
                phase_three_h<BLOCK_SIZE><<< p3b_g21, dimt >>>(device_ptr, pitch, pivot, sp[0], 0, vert, sp[0], vert, r);
                pivot += BLOCK_SIZE;
            }
            ...
            LOG("[mpi %d] isend to %d", 1, 0);
            MPI_Isend(data+sp[0]*vert, vert*sp[1], MPI_INT, 0, 1, MPI_COMM_WORLD, &req);
        ...
        if(num==0){  //G11 relax
            ...
            int pivot = sp[0];
            for(int r=0;r<Round_1;++r){
                phase_three<BLOCK_SIZE><<< p3b_g11 , dimt >>>(device_ptr, pitch, pivot, 0, sp[0], vert, vert);
                pivot += BLOCK_SIZE;
            }
